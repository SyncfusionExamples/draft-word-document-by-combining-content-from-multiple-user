...
function initializeTitleBar(isShareNeeded, isRtl) {
    ...
    download = addButton('e-de-icon-Download ' + iconCss, downloadText, btnStyles, 'documenteditor-share', downloadToolTip, false);
    if (!isShareNeeded) {
...
function wireEventsInTitleBar() {
    ...
    download.element.addEventListener('click', onSave);
...
function onSave() {
    ...
    http.open('POST','http://localhost:51056/api/documenteditor/ExportSFDT');
    http.setRequestHeader('Content-Type', 'application/json;charset=UTF-8');
    http.responseType = 'json';
    //Serialize document content as SFDT.
    let sfdt = { Content: documenteditor.serialize(), FileName: "Getting Started.docx", UserName: userName };
    //Send the sfdt content to server side.
    http.send(JSON.stringify({ Content: documenteditor.serialize(), FileName: "Getting Started.docx", UserName: userName }));
}
